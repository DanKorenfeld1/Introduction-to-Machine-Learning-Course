...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        for t in range(self.max_iter_):
            ...
            eta = self.learning_rate_.lr_step(f=f, X=X, dx=-jacob, t=t)  # the size of the step
            eta_mult_jacob = eta * jacob
            w_t_plus_1 = w_t - eta_mult_jacob
            delta = np.linalg.norm(w_t_plus_1 - w_t)

            f.weights = w_t_plus_1
            ...
            if delta < self.tol_:  # when the update too small
                break
            self.callback_(solver=self, weight=np.copy(f.weights), val=output, grad=jacob, t=t, eta=eta, delta=delta)
        ...
        avg_weights = sum_weights / max_t  # max_t>0
        return avg_weights
