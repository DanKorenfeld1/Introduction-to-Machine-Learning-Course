...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        # init the random numbers with standard gaussian distribution
        init = np.random.randn(X.shape[1]) / (X.shape[1] ** 0.5)
        if self.penalty_ == "none":
            objective = LogisticModule(weights=init)
        else:
            regularization_module = {"l1": L1, "l2": L2}[self.penalty_]()
            objective = RegularizedModule(fidelity_module=LogisticModule(),
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        x_mult_coef = X @ self.coefs_
        exp_mult_x = np.exp(x_mult_coef)
        return exp_mult_x / (1 + exp_mult_x)  # the sigmoid of (Xw)
